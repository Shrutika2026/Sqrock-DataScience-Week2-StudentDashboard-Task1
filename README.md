🎓 Student AI Performance Dashboard

📌 Quick Summary
An interactive Data Science product that analyzes student test scores and uses a Linear Regression model to predict Math results based on Reading and Writing performance.

🖼️ Project Gallery
1. Live Dashboard
   https://sqrock-datascience-week2-studentdashboard-task1-jgxbpk8dtgxuzv.streamlit.app/

3. ML Model Training (Colab)
   https://colab.research.google.com/drive/1GuQ92kmoplvr3zAKP4cxNgel-7ySq5nY?usp=sharing

🛠️ Tech Stack
Analytics: Python, Pandas

Visuals: Plotly, Seaborn

AI Model: Scikit-Learn (Linear Regression)
<img width="3504" height="2426" alt="image" src="https://github.com/user-attachments/assets/c18477b7-392d-486a-b9a5-10882424ccfa" />

<img width="695" height="470" alt="image" src="https://github.com/user-attachments/assets/b587ed15-5cf1-444d-a5b7-82f7fe496d61" />

<img width="533" height="435" alt="image" src="https://github.com/user-attachments/assets/4fc96683-cf27-4c79-8c1c-e01d546857d5" />




Interface: Streamlit

🚀 Key Features
EDA: Automated cleaning and handling of missing data.

Visuals: Distribution histograms and correlation heatmaps.

AI Predictor: Real-time Math score prediction with a confidence gauge.

Performance: Model achieved a 0.68 R2 Score.

⚙️ Setup
pip install -r requirements.txt

streamlit run app.py
